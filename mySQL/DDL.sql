...
CREATE TABLE IF NOT EXISTS `railway`.`passenger` (
  ...
  `cel` VARCHAR(20) NULL DEFAULT NULL,
  `id_flightbooking` INT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_flightbooking_id`
    FOREIGN KEY (`id_flightbooking`)
    REFERENCES `railway`.`flightbooking` (`id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;

CREATE INDEX `fk_flightbooking_id` ON `railway`.`passenger` (`id_flightbooking` ASC) VISIBLE;
...
-- -----------------------------------------------------
-- procedure CheckIdExists
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `CheckIdExists`(
...
-- -----------------------------------------------------
-- procedure CheckIdExistsINT
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `CheckIdExistsINT`(
	IN tableName VARCHAR(64), 
    IN columnName VARCHAR(64), 
    IN idValue INT
)
...
-- -----------------------------------------------------
-- procedure CheckIdExistsString
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `CheckIdExistsString`(
	IN tableName VARCHAR(64), 
    IN columnName VARCHAR(64), 
    IN idValue VARCHAR(64)
)
...
DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `GetAllEmployees`()
BEGIN
    SELECT
        e.id,
        e.name,
...
-- -----------------------------------------------------
-- procedure HasFlightConnections
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `HasFlightConnections`(
    IN flightId INT, 
...
-- -----------------------------------------------------
-- procedure HasFlightCrewAssigns
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `HasFlightCrewAssigns`(
...
DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `calculateTotalPrice`(
    IN p_flight_id INT,
    IN p_fare_id INT,
...
BEGIN
    ...
    SELECT price_trip INTO flight_price
    FROM flight
    WHERE id = p_flight_id;

    -- Obtener el precio de la tarifa
    SELECT value INTO fare_price
    FROM flightfare
    WHERE id = p_fare_id;

    -- Calcular el precio total
    SET total_price = flight_price + fare_price;
END$$

DELIMITER ;

-- -----------------------------------------------------
-- procedure checkAndInsertCustomer
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `checkAndInsertCustomer`(
    IN p_id VARCHAR(10),
...
-- -----------------------------------------------------
-- procedure checkAndInsertPassenger
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `checkAndInsertPassenger`(
...
BEGIN
    ...
    IF passengerExists = 0 THEN
        INSERT INTO passenger (name, id, birthDate, cel, id_flightbooking)
        VALUES (p_name, p_idNumber, p_birthDate, p_cel, p_id_flightbooking);
    END IF;
END$$
...
DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `createBooking`(
...
BEGIN
    INSERT INTO customer (id, name, age, doc_type) 
    VALUES (id_insert, name_insert, age_insert, doc_type);
END$$
...
-- -----------------------------------------------------
-- procedure createFlightCrew
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `createFlightCrew`(
...
-- -----------------------------------------------------
-- procedure flightConnectionCreator
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `flightConnectionCreator`(
...
DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `monda`(
)
BEGIN
	SELECT * FROM plane;
END$$
...
-- -----------------------------------------------------
-- procedure showAirportByFlightDate
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `showAirportByFlightDate`(
    IN Dateinserted VARCHAR(64)
)
BEGIN
    SET @query = CONCAT('
        SELECT
        ...
        FROM
            airport a
        JOIN
            city c ON a.idcity = c.id
        JOIN
            flight f ON c.id = f.orig_city
        WHERE
            f.trip_date = "', Dateinserted, '"'
    );
...
-- -----------------------------------------------------
-- procedure showFlightsByAirpDate
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `showFlightsByAirpDate`(
...
-- -----------------------------------------------------
-- procedure showFlightsByDate
-- -----------------------------------------------------

DELIMITER $$
USE `railway`$$
CREATE DEFINER=`root`@`%` PROCEDURE `showFlightsByDate`(
